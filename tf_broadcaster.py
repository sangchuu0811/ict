...
def load_map(self, pgm_file_path, yaml_file_path):
    ...
    try:
        ...
        self.image_shape = cropped_image.shape
...
def update_visualization_markers(self, coordinates):
    ...
    self.get_logger().info(f"x_min 좌표값: {x_min}")
    self.get_logger().info(f"크롭된 이미지 범위: x_min={x_min}, y_min={y_min}, x_max={x_max}, y_max={y_max}")
    ...
    for i, (lat, lon) in enumerate(coordinates):
        ...
        self.get_logger().info(f"발행된 마커: id={visual_marker.id}, position=({visual_marker.pose.position.x}, {visual_marker.pose.position.y}, {visual_marker.pose.position.z})")
