...
# 기본 GPS 좌표 설정
# latitude = 37.631679 # 보드에서 gps센서가 보내주는 위경도 값을 latitude, longitude에 저장
# longitude = 127.054740
# altitude = 0.0

latitude = 0.0 # 보드에서 gps센서가 보내주는 위경도 값을 latitude, longitude에 저장
longitude = 0.0
...
class TfBroadcaster(Node):
    ...
    def sensor_marker(self): # gps 센서의 위치 마커를 찍는 함수(red)
        ...
        sen_visual_marker.pose.position = Point(x=longitude, y=latitude, z=1.0)
        ...
        # 마커의 중심계산 (x=0 y=0)
        center_pixel_x = int((self.origin[0]-longitude)/self.resolution)
        center_pixel_y = int((self.origin[1]-latitude)/self.resolution)
    ...
    # 마커의 위치를 픽셀 좌표로 변환
        ...
        # 원점을 마커의 위치로 설정
        grid_msg.info.origin.position.x = longitude + 0.001 #자른 pgm중간에 마커 찍기 
        grid_msg.info.origin.position.y = latitude + 0.001 #자른 pgm중간에 마커 찍기 
    ...
    def timer_callback(self):
        global latitude, longitude  # 글로벌 변수로 위도와 경도 사용
        altitude = 0.0

        # GPS 데이터 발행
        if latitude is not None and longitude is not None:  # 위도와 경도가 None이 아닐 경우
            msg = NavSatFix()
