...
class TfBroadcaster(Node):
    ...
    def update_visualization_markers(self, coordinates):
        ...
        center_pixel_x = int((self.origin[0]-lon)/self.resolution)
        center_pixel_y = int((self.origin[1]-lat)/self.resolution)
        ...
        crop_size = 50
        # x_min = (int((lon-self.origin[0])/self.resolution))
        # x_max = 25
        # y_min = (int((lat-self.origin[1])/self.resolution))
        # y_max = 25
    ...
    def publish_cropped_map(self, x_offset, y_offset):
        ...
        grid_msg.info.resolution = self.resolution
        # grid_msg.info.origin.position.x = self.origin[0] + (x_offset + (self.cropped_image_shape[1] // 2)) * self.resolution  # 잘라낸 이미지의 원점
        # grid_msg.info.origin.position.y = self.origin[1] + (y_offset + (self.cropped_image_shape[0] // 2)) * self.resolution
        # grid_msg.info.origin.position.z = self.origin[2]

        # grid_msg.info.origin.position.x = self.origin[0] + (x_offset * self.resolution)
        # grid_msg.info.origin.position.y = self.origin[1] + (y_offset * self.resolution)
        # grid_msg.info.origin.position.z = self.origin[2]
    ...
    # 마커의 위치를 픽셀 좌표로 변환
        ...
        # 원점을 마커의 위치로 설정
        grid_msg.info.origin.position.x = self.origin[0]
        grid_msg.info.origin.position.y = self.origin[1] + (marker_pixel_y - (self.cropped_image_shape[0] // 2)) * self.resolution
        grid_msg.info.origin.position.z = 0.000
